 ...
 * Last Modified: February 3th, 2015
 * @version 1.95
 * @package PegasusPHP
 *
 * Copyright (C) 2008-2015 Chris Schuld  (chris@chrisschuld.com)
 *
...
class Browser {
	...
	const BROWSER_YANDEX = 'Yandex';				// http://browser.yandex.ru/
	...
	protected function checkBrowserOpera() {
		if( stripos($this->_agent,'opera mini') !== false ) {
			...
			if( preg_match('/Version\/(10.*)$/',$resultant,$matches) ) {
				...
				$aversion = explode(' ',stristr($resultant,'opr'));
				$this->setVersion(isset($aversion[1])?$aversion[1]:"");
			}
			$this->_browser_name = self::BROWSER_OPERA;
			return true;
