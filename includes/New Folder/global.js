...
function ValidateResumeTitle(p_field) {
	if(Trim(p_field).value == "" ) {
		alert("Please Enter Resume Title.");            
		p_field.focus();
		return 0;
	}else if(CheckSpecialCharactersResume(p_field)){
		return 0;
	}else{
		return 1;
	}
...
function CheckSpecialCharactersResume(p_field){
	 var Character;
	 for(var i=0; i < p_field.value.length; i++){
		 Character = p_field.value.substring(i,i+1);
		 if(Character=='#' || Character=='~' || Character=='`' || Character=='"' || Character=='?' || Character=='&' || Character=='<' || Character=='>'){
			alert("Please do not enter special characters for Resume Title.");
			p_field.focus();
			return 1;	
			break;
		 }
