<? if (empty($_SESSION['UserName']) && empty($_SESSION['MemberID'])) { ?>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
				 <tr>
                  ...
                  <tr>
					...
					<table width="96%" border="0" cellspacing="0" cellpadding="4">
						...
						<tr>
						<td align="left" valign="middle"><a href="<?=$Config['Url']?>logout.php?Store=<?=$_SESSION['StoreUserName']?>" class="forgot_link"><?=LOG_OUT?></a></td>
						</tr>
		...
		<?  if($_SESSION['MemberID']!=$_SESSION['StoreID']){		   ?>	
						...
						<td align="left" valign="middle">
						<?
						echo '<a class="forgot_link" href="'.$Config['Url'].'subscribe.php?Type=email&MemberID='.$_SESSION['MemberID'].'&StoreID='.$_SESSION['StoreID'].'&pId='.$_GET['id'].'&UserName='.$_SESSION['StoreUserName'].'" onclick="return confDel(\''.SUSBCRIBE_FOR_EMAIL_ALERT.'\')">'.SUSBCRIBE_FOR_EMAIL.'</a> ';
						?>		
						...
						<td align="left" valign="middle">
						<?
						echo ' <a class="forgot_link" href="'.$Config['Url'].'subscribe.php?Type=sms&MemberID='.$_SESSION['MemberID'].'&StoreID='.$_SESSION['StoreID'].'&pId='.$_GET['id'].'&UserName='.$_SESSION['StoreUserName'].'" onclick="return confDel(\''.SUSBCRIBE_FOR_SMS_ALERT.'\')">'.SUSBCRIBE_FOR_SMS.'</a> ';
						?>
						</td>
						</tr>
						<? } ?>
