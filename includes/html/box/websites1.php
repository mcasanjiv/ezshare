...
if(empty($WebsiteTitle))
	$WebsiteTitle = ($_GET['SearchBy']!='Website')?(FEATURED_WEBSITES):(WEBSITES);
...
<div class="header_txt" ><?=$WebsiteTitle?></div>
  <? if($_GET['SearchBy']!='Website' && $num>8) { ?><a href="websites.php?SearchBy=Website" class="whitetxt_link">View All</a> <? } ?></td>
</tr>
<?
 ...
 <? 
  ...
  foreach($arryWebsite as $key=>$values){
   $i++;
	...
	if($WebsiteTitle == FEATURED_STORES && $i==9) break;
