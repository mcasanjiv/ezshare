<?php   
	...
	if(!empty($Email)){
		...
		$StorageLimit = $arryXml['subscriptiondetail']['space']['value'];
		$StorageLimitUnit = $arryXml['subscriptiondetail']['space_unit']['value'];
		$ValidXml=1;
	}else{	/*
	...
	$arryCompany['Department'] = 5;
	$arryCompany['DisplayName'] = $DisplayName;
	$arryCompany['Email'] = $Email;
	$arryCompany['Password'] = $Password;
	$arryCompany['CompanyName'] = $CompanyName;
	$arryCompany['ContactPerson'] = $ContactPerson;
	$arryCompany['Address'] = $Address;
	$arryCompany["OtherCity"]= $City;
	$arryCompany["OtherState"]= $Country;
	$arryCompany["Country"]= $Country;
	$arryCompany["ZipCode"]= $ZipCode;
	$arryCompany["Status"]= '0';
	$arryCompany["Timezone"]= '-04:00';
	$arryCompany["DateFormat"]= 'j M, Y';
	$arryCompany["currency_id"]= '9';
	$arryCompany["MaxUser"]= $MaxUser;
	$arryCompany['PaymentPlan'] = $PaymentPlan;
	$arryCompany['StorageLimit'] = $StorageLimit;
	$arryCompany['StorageLimitUnit'] = $StorageLimitUnit;
	/**************************
	$NumMonth = 12;
	...
	/**************************/
	$ExpiryDate = date("Y-m-d",$enddate);
	$arryCompany["ExpiryDate"]= $ExpiryDate;
	...
	$objCompany=new company();
	 ...
	 if(!empty($arryCompany['Email']) && !empty($arryCompany['DisplayName'])) {
		//echo print_r($arryCompany);exit;					
		if($objConfig->isCmpEmailExists($arryCompany['Email'],'')){
			$_SESSION['mess_company'] = EMAIL_ALREADY_REGISTERED;
		}else if($objCompany->isDisplayNameExists($arryCompany['DisplayName'],'')){
			$_SESSION['mess_company'] = DISPLAY_ALREADY_REGISTERED;
		}else{	

			/************************/
			if(!empty($arryCompany["Country"])){
				$arryCountry = $objRegion->GetCountryID($arryCompany["Country"]);  
				$arryCompany["country_id"]=$arryCountry[0]['country_id']; //set	
				if($arryCountry[0]['country_id']>0 && !empty($arryCompany["OtherState"])){		
					$arryState = $objRegion->GetStateID($arryCompany["OtherState"], $arryCountry[0]['country_id']); 
					$arryCompany["main_state_id"]=$arryState[0]['state_id'];//set
				}
				if($arryCountry[0]['country_id']>0 && !empty($arryCompany["OtherCity"])){		
					$arryCity = $objRegion->GetCityID($arryCompany["OtherCity"], $arryCountry[0]['country_id']); 
					$arryCompany["main_city_id"]=$arryCity[0]['city_id'];//set
				}
			}
			/************************/
			//print_r($arryCompany);exit;
			$CmpID = $objCompany->AddCompany($arryCompany); 
		...
		/************************/
		if($AddDatabase == 1){
			$DbName = $objCompany->AddDatabse($arryCompany['DisplayName']); 
			if(!empty($DbName)){
		...
		if($UpdateAdminMenu == 1){					
			...
			$objCompany->UpdateAdminModules($CmpID,$arryCompany['Department']);	

			$objCompany->UpdateAdminSubModules($CmpID,$arryCompany['Department'],$PaymentPlan);				
		}
		/************************/
		print $ValidXml; exit;				
