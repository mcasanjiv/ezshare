       ...
       function  GetDashboardLead($AdminType,$EmpID)
		{
			...
			$strSQLQuery .= ($Config['vAllRecord']!=1)?(" and (l.AssignTo like '%".$_SESSION['AdminID']."%' OR l.created_id='".$_SESSION['AdminID']."') "):("");
			//$strAddQuery .= ($_SESSION['AdminType']!="admin")?(" and l.AssignTo='".$_SESSION['AdminID']."' "):("");
		...
		{  
		 ...
		 $strSQLQuery = "insert into c_lead (LeadName,type,ProductID,product_price, primary_email,company,Website,FirstName,LastName,Address, city_id, state_id, ZipCode, country_id,Mobile, LandlineNumber,lead_status,lead_source, JoiningDate,  OtherState, OtherCity,  ipaddress, UpdatedDate,AssignTo,AnnualRevenue,designation,description,Industry,LeadDate , created_by, created_id,NumEmployee) values('".addslashes($LeadName)."','".addslashes($type)."','".addslashes($ProductID)."', '".addslashes($product_price)."','".addslashes($primary_email)."', '".addslashes($company)."','".addslashes($Website)."','".addslashes($FirstName)."', '".addslashes($LastName)."', '".addslashes($Address)."',  '".$main_city_id."', '".$main_state_id."','".addslashes($ZipCode)."', '".$country_id."', '".addslashes($Mobile)."','".addslashes($LandlineNumber)."','".addslashes($lead_status)."','".addslashes($lead_source)."',  '".$JoiningDatl."',  '".addslashes($OtherState)."', '".addslashes($OtherCity)."','".$ipaddress."',  '".$Config['TodayDate']."','".addslashes($AssignTo)."','".addslashes($AnnualRevenue)."','".addslashes($designation)."','".addslashes($description)."','".addslashes($Industry)."','".addslashes($LeadDate)."', '".addslashes($_SESSION['AdminType'])."', '".addslashes($_SESSION['AdminID'])."','".addslashes($NumEmployee)."')";
			...
			$contents = str_replace("[FULLNAME]",$LeadName, $contents);
...
$strAddQuery = ($arrayDetail!='')?(" and  e.EmpID in (".$arrayDetail.")"):("");
		 ...
		 $strSQLQuery = "select e.*,d.Department as emp_dep,d.depID from h_employee e left outer join  h_department d on e.Department=d.depID  WHERE e.Status=1 and e.Division in (5) ".$strAddQuery;
		return $this->query($strSQLQuery, 1);
...
function  GetNumTicket($priority)
	{
		...
		//$strSQLQuery .= ($_SESSION['AdminType']!="admin")?(" where l.AssignTo=".$EmpID):(" where 1 ");
		//$strAddQuery .= ($_SESSION['AdminType']!="admin")?(" and l.AssignTo='".$_SESSION['AdminID']."' "):("");
		//$strAddQuery .= ($Opportunity!=1)?(" and l.Opportunity='".$Opportunity."' "):("");
	...
	$strSQLQuery .= ($Config['vAllRecord']!=1)?(" and (c.assignedTo like '%".$_SESSION['AdminID']."%' OR c.created_id='".$_SESSION['AdminID']."') "):("");
	//$strSQLQuery .= ($_SESSION['AdminType']!="admin")?(" where l.AssignTo=".$EmpID):(" where 1 ");
	//$strAddQuery .= ($_SESSION['AdminType']!="admin")?(" and l.AssignTo='".$_SESSION['AdminID']."' "):("");
	//$strAddQuery .= ($Opportunity!=1)?(" and l.Opportunity='".$Opportunity."' "):("");
