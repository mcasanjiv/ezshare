...
class common extends dbClass
{
		...
		function  GetAttributeValue($attribute_name,$OrderBy)
		{
			...
			$strSQLQuery = "select v.* from p_attribute_value v inner join p_attribute a on v.attribute_id = a.attribute_id ".$strSQLFeaturedQuery.$OrderSql;
		...
		function  GetAttributeByValue($attribute_value,$attribute_name)
		{
			...
			$strSQLQuery = "select v.attribute_value from p_attribute_value v inner join p_attribute a on v.attribute_id = a.attribute_id ".$strSQLFeaturedQuery;
		...
		function  AllAttributes($id)
		{
			...
			$strSQLQuery = "select * from p_attribute ".$strSQLQuery." order by attribute_id Asc" ;
		...
		{
			@extract($arryAtt);	 
			$sql = "insert into p_attribute_value (attribute_value,attribute_id,Status,locationID) values('".addslashes($attribute_value)."','".$attribute_id."','".$Status."','".$_SESSION['locationID']."')";
			$rs = $this->query($sql,0);
		...
		{
			@extract($arryAtt);	
			$sql = "update p_attribute_value set attribute_value = '".addslashes($attribute_value)."',attribute_id = '".$attribute_id."',Status = '".$Status."'  where value_id = ".$value_id; 
			$rs = $this->query($sql,0);
		...
		function getAttribute($id=0,$attribute_id,$Status=0)
		{
			...
			$sql = "select * from p_attribute_value ".$sql." order by value_id asc" ;
		
			return $this->query($sql, 1);
		}
		function countAttributes()
		{
			$sql = "select sum(1) as NumAttribute from p_attribute_value where Status=1" ;
			return $this->query($sql, 1);
		}

		function changeAttributeStatus($value_id)
		{
			$sql="select * from p_attribute_value where value_id=".$value_id;
			$rs = $this->query($sql);
			if(sizeof($rs))
			{
				if($rs[0]['Status']==1)
				...
				$sql="update p_attribute_value set Status='$Status' where value_id=".$value_id;
				$this->query($sql,0);
				return true;
			}			
		}
		...
		{
			$sql = "delete from p_attribute_value where value_id = ".$id;
			$rs = $this->query($sql,0);
		...
		function isAttributeExists($attribute_value,$attribute_id,$value_id)
			{
				...
				$strSQLQuery ="select value_id from p_attribute_value where LCASE(attribute_value)='".strtolower(trim($attribute_value))."' and locationID=".$_SESSION['locationID'];
		...
		function isAttribExists($attribute_value,$attribute_id,$value_id)
		{
				...
				$strSQLQuery ="select value_id from p_attribute_value where LCASE(attribute_value)='".strtolower(trim($attribute_value))."' ";
		...
		function  ListTerm($arryDetails)
		{
			...
			$strSQLQuery = "select * from p_term  ".$strAddQuery;
		...
		function  GetTerm($termID,$Status)
		{
			...
			$strSQLQuery = "select * from p_term  ".$strAddQuery." order by termID Asc";
		...
		{  
			...
			$strSQLQuery = "insert into p_term (termName, termDate, Due, Status, Day, CreditLimit, UpdatedDate ) values( '".addslashes($termName)."', '".$termDate."', '".addslashes($Due)."', '".$Status."', '".addslashes($Day)."', '".addslashes($CreditLimit)."',  '".$Config['TodayDate']."')";
		...
		function UpdateTerm($arryDetails){
			...
			$strSQLQuery = "update p_term set termName='".addslashes($termName)."', termDate='".$termDate."',  Due='".addslashes($Due)."',  Status='".$Status."'  ,Day='".addslashes($Day)."'	,CreditLimit='".addslashes($CreditLimit)."'	, UpdatedDate = '".$Config['TodayDate']."' where termID=".$termID; 
		...
		function RemoveTerm($termID)
		{
			...
			$strSQLQuery = "delete from p_term where termID=".$termID; 
			$this->query($strSQLQuery, 0);			
		...
		{
			$sql="select * from p_term where termID=".$termID;
			$rs = $this->query($sql);
			if(sizeof($rs))
			{
				if($rs[0]['Status']==1)
				...
				$sql="update p_term set Status='$Status' where termID=".$termID;
				$this->query($sql,0);				
		...
		{
			$sql="select termID from p_term where termID in (".$termIDs.") and Status!='".$Status."'"; 
			$arryRow = $this->query($sql);
			if(sizeof($arryRow)>0){
				$sql="update p_term set Status='".$Status."' where termID in (".$termIDs.")";
				$this->query($sql,0);			
			}	
			return true;
		}
		...
		function isTermExists($termName,$termID=0)
		{
			...
			$strSQLQuery = "select termID from p_term where LCASE(termName)='".strtolower(trim($termName))."'".$strSQLQuery; 
			$arryRow = $this->query($strSQLQuery, 1);
