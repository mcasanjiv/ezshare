...
class category extends dbClass
{
		...
		function  GetCategory($CategoryID)
		{
			...
			$strSQLQuery = "select * from e_categories WHERE CategoryID = ".$CategoryID;  
			return $this->query($strSQLQuery, 1);
		}
		...
		function getCategories($parentid,$num,$selected=0) {
			    ...
			    $arrayRows = $this->query($strSQLQuery, 1);
                    ...
                    function getAppliedAttributeCategory($attrID)
                    {
                            ...
                            $strSQLQuery = "SELECT * FROM e_catalog_attributes WHERE Gaid='".$attrID."'";
		...
		{
			 $strSQLQuery = "update e_categories set ".$FieldName."='".$imageName."' where CategoryID=".$CategoryID;
		...
		function UpdateCategory($arryDetails)
		{
			extract($arryDetails);
			  ...
			  $strSQLQuery = "update e_categories set ParentID='".$ParentID."',Name='".addslashes($Name)."',MetaTitle='".addslashes($MetaTitle)."',MetaKeyword='".addslashes($MetaKeyword)."', MetaDescription='".addslashes($MetaDescription)."', CategoryDescription='".addslashes($CategoryDescription)."',sort_order='".$sort_order."', Image='', Status='".$Status."' where CategoryID=".$CategoryID;
		     	  ...
		     	  $strSQLQuery = "update e_categories set ParentID='".$ParentID."',Name='".addslashes($Name)."',MetaTitle='".addslashes($MetaTitle)."',MetaKeyword='".addslashes($MetaKeyword)."', MetaDescription='".addslashes($MetaDescription)."', CategoryDescription='".addslashes($CategoryDescription)."',sort_order='".$sort_order."', Status='".$Status."' where CategoryID=".$CategoryID;
                        }
			$this->query($strSQLQuery, 0);
		...
		function RemoveCategoryCompletly($id)
		{
			...
			$strSQLQuery = "delete from e_categories where ParentID=".$id; 
			$this->query($strSQLQuery, 0);
		...
		{
			$sql="select * from e_categories where CategoryID=".$CategoryID;
			$rs = $this->query($sql);
			if(sizeof($rs))
			{
				if($rs[0]['Status']==1)
				...
				$sql="update e_categories set Status='$Status' where CategoryID=".$CategoryID;
				$this->query($sql,0);
				return true;
			}			
		}
		...
		{
			$strSQLQuery ="select * from e_categories where ParentID=".$id;
			$arryRow = $this->query($strSQLQuery, 1);
		...
		{
			$strSQLQuery ="select * from e_products where CategoryID=".$id;
			$arryRow = $this->query($strSQLQuery, 1);
			if (!empty($arryRow[0]['CategoryID'])) {
				return true;
			} else {
		...
		function isCategoryExists($Name,$CategoryID=0,$ParentID=0)
		{
			...
			$strSQLQuery ="select CategoryID from e_categories where LCASE(Name)='".strtolower(trim($Name))."' and ParentID = ".$ParentID;
		...
		function GetNumProductsMultiple($CategoryID,$PostedByID){
			...
			$strSQLQuery = "select count(*) as NumProducts from e_products p1 where p1.Status=1  and p1.CategoryID in( select CategoryID from e_categories where ParentID='".$CategoryID."' and Status=1)".$strAddQuery;
