...
class payroll extends dbClass
{
		...
		function addSalary($arryDetails){ 
			...
			/********************/
			@extract($arryDetails);	 
			$sql = "insert into h_salary(EmpID, catEmp, CTC, Gross, NetSalary, SalaryData, updatedDate, BankName, AccountName, AccountNumber, IFSCCode) values('".$EmpID."', '".$catEmp."', '".$CTC."', '".$Gross."', '".$NetSalary."', '".$SalaryData."', '".$Config['TodayDate']."', '".addslashes($BankName)."', '".addslashes($AccountName)."', '".addslashes($AccountNumber)."', '".addslashes($IFSCCode)."')";
			$this->query($sql,0);
			return true;
		}
	...
	function updateReturnLoan($LoanData){
		...
		function getTotalBonus($EmpID=0,$Year,$Month)
		{
			if(!empty($EmpID) && !empty($Year) && !empty($Month) ){
				$sql = "select a.Amount,a.BonusID  from h_bonus a inner join h_employee e on a.EmpID=e.EmpID where a.Approved=1 and a.Paid='0' and a.EmpID = '".mysql_real_escape_string($EmpID)."' and a.Year = '".mysql_real_escape_string($Year)."' and a.Month = '".mysql_real_escape_string($Month)."' "; 
				return $this->query($sql, 1);
			}
		}
