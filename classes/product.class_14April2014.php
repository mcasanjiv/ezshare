...
class product extends dbClass
{
		...
		{        
                       global $Config; 
			$sql="update e_products set ViewedDate='".$Config['TodayDate']."' where ProductID=".$ProductID;
			$this->query($sql,0);
			return true;
		}
		...
		{	
			extract($arryDetails);  
                        $promo_products = $promo_products == "" ? array() : explode(",", $promo_products);
                         ...
                         $strSQLQuery =  "DELETE FROM e_recommended_products WHERE ProductID='".$ProductID."'";
                         $this->query($strSQLQuery, 0);
			...
			return 1;
                ...
                function updateGlobalAttribute($arryDetails)
                {
                    ...
                    extract($arryDetails);
                        ...
                        $strSQLQuery = "delete from e_products_attributes where gaid=".$Gaid;
                        $this->query($strSQLQuery,0);
                        $strSQLQuery = "delete from e_catalog_attributes where Gaid=".$Gaid;
                        $this->query($strSQLQuery,0);
			...
			$strSQLQuery = "UPDATE e_global_attributes set AttributeType='".trim($attribute_type)."',IsGlobal='".$IsGlobal."', TextLength = '20', Status='".trim($is_active)."',Priority='".trim($priority)."',Name='".trim($attname)."',Caption='".trim($attname)."',Options='".trim($options)."' WHERE Gaid = '".$Gaid."'";
			$this->query($strSQLQuery, 0);
			...
			if(sizeof($rs))
			{
				...
				$strSQLQuery = "update e_global_attributes set Status='$Status' where Gaid=".$attrID;
				$this->query($strSQLQuery,0);
			...
			return true;
                ...
                {
                    $strSQLQuery = "delete from e_global_attributes where Gaid=".$attrID;
		    $this->query($strSQLQuery,0);
                    $strSQLQuery = "delete from e_products_attributes where gaid=".$attrID;
		    $this->query($strSQLQuery,0);
                    $strSQLQuery = "delete from e_catalog_attributes where Gaid=".$attrID;
		    $this->query($strSQLQuery,0);
                }
                ...
                function GetProductAttributesForFront($ProductID)
		{
			...
			$strSQLQuery = "Select * from e_products_attributes where pid=".$ProductID." AND is_active='Yes' order by priority asc";
  			return $this->query($strSQLQuery, 1);
		}
...
/*************************************Alternatative Images Function Start******************************************************************/ 
		function RemoveProduct($id,$CategoryID,$Front)
		{
			...
			$this->query($strSQLQuery, 0);
                        ...
                        $strSQLQuery = "delete from e_products_categories where pid=".$id; 
			$this->query($strSQLQuery, 0);
