...
class orders extends dbClass {
    ...
    function GetMyOrders($Cid, $SearchKey, $SortBy, $AscDesc) {
        $strAddQuery = '';

        $strAddQuery .= (!empty($Cid)) ? (" where o.Cid=" . $Cid) : (" where 1");

        $strAddQuery .= (!empty($SearchKey)) ? (" and (LCASE(o.BillingName) like '" . $SearchKey . "%' or LCASE(o.OrderDate) like '" . $SearchKey . "%')") : ("");

        $strAddQuery .= (!empty($SortBy)) ? (" order by " . $SortBy . " ") : (" order by o.OrderID ");
        $strAddQuery .= (!empty($AscDesc)) ? ($AscDesc) : (" Desc");

        $strSQLQuery = "select o.*,m.UserName,m.CompanyName,cr.code,cr.symbol_left,cr.symbol_right from e_orders o left outer join members m on o.StoreID=m.Cid left outer join currencies cr on o.currency_id=cr.currency_id" . $strAddQuery;
        return $this->query($strSQLQuery, 1);
    }
    ...
    function getOrderedProductById($oid) {
        ...
        $strSQLQuery = "SELECT od.*,o.OrderID,p.ProductSku FROM e_orderdetail od INNER JOIN e_orders o  on od.OrderID=o.OrderID LEFT OUTER JOIN e_products p on od.ProductID=p.ProductID WHERE od.OrderID=".$oid."";
       // echo $strSQLQuery;exit;
        return $this->query($strSQLQuery, 1);
    }
    ...
    function saveOrderStatus($arryDetails)
    {
        ...
        {
            $strSQLQuery = "UPDATE e_orders SET OrderStatus = '".$order_status."' WHERE OrderID = '".$oid."'";
            $this->query($strSQLQuery, 0);   
        }
    ...
    function AdminData() {
        $strSQLQuery = "select Tax,Shipping,AdminEmail from admin";
        return $this->query($strSQLQuery, 1);
    }
    ...
    function AddToCart($ProductID, $Cid, $Quantity, $attributes) {
                    ...
                    if ($arryRowAttr["is_modifier"] == "Yes") {
                        ...
                        if (strlen($modifier) > 2) {
                            if ($modifier[strlen($modifier) - 1] == ")") {
                                ...
                                $option_price = $product['Price'] + $price_modifier;
                                ...
                                $option_weight = $product['Weight'] + $weight_modifier;
        ...
        $Price = $product['Price'] + $totalAttrPrice;
        $Weight = $product['Weight'] + $totalAttrWeight;
        //echo "=>".$Weight;exit;
        ...
        if (!empty($arryRow[0]['CartID'])) {
            $strSQLQuery = "update e_cart set Quantity=Quantity+" . $Quantity . ", Options = '" . $selected_options . "', Weight = '" . $Weight . "', AddedDate='" . date('Y-m-d') . "' where CartID='" . $arryRow[0]['CartID'] . "' AND ProductID = '" . $ProductID . "'";
            //echo "=>".$strSQLQuery;exit;
            $this->query($strSQLQuery, 0);
        } else {
            ...
            $strSQLQuery = "insert into e_cart (Cid,ProductID,Price,PriceBeforeQuantityDiscount,Quantity,IsTaxable,TaxClassId,FreeShipping, Options, Weight,AddedDate) values('" . $Cid . "', '" . $ProductID . "', '" . $Price . "','" . $Price . "','" . $Quantity . "', '" . $product['IsTaxable'] . "', '" . $product['TaxClassId'] . "', '" . $FreeShipping . "', '" . $selected_options . "', '" . $Weight . "','" . date('Y-m-d') . "')";
            $this->query($strSQLQuery, 0);
        }
    ...
    public function checkQuantityDiscounts($Cid) {
        ...
        //echo "=>"."SELECT SUM(c.quantity) AS q, c.ProductID FROM e_cart AS c WHERE c.Cid = '".$Cid."' GROUP BY c.ProductID";
        //echo "=>". $strSQLQuery;exit;
        //$arryRow = $this->query($strSQLQuery, 1);
        // echo "=>".mysql_num_rows($strSQLQuery);
        $arryRow = $this->query($strSQLQuery, 1);
        //echo "<pre>";
        //print_r($arryRow);
        //echo "=>".count($arryRow);exit;
        ...
        for ($i = 1; $i <= $numPriceProduct; $i++) {
            ...
            if ($ProductID > 0) {
                ...
                if ($Cid == $_SESSION['StoreID']) {
                    ...
                    if (!empty($arryRow[0]['CartID'])) {
                        $strSQLQuery = "update e_cart set Quantity='" . $Quantity . "',AddedDate='" . date('Y-m-d') . "' where CartID='" . $arryRow[0]['CartID'] . "'";
                        $this->query($strSQLQuery, 0);
                    } else {
                        $strSQLQuery = "insert into e_cart (Cid,ProductID,Price,Tax,Quantity,StoreID,AddedDate) values('" . $Cid . "', '" . $ProductID . "', '" . $Price . "','" . $Tax . "','" . $Quantity . "','" . $_SESSION['StoreID'] . "','" . date('Y-m-d') . "')";
                        $this->query($strSQLQuery, 0);
                    }
                }
            }
    ...
    function AddOrder($arryDetails) {
        ...
        $strSQLQuery = "insert into e_orders SET Cid = '" . $Cid . "', ProductIDs = '" . $ProductIDs . "', currency_id = '" . $currency_id . "', Currency = '" . $Config['Currency'] . "',CurrencySymbol = '" . $Config['CurrencySymbol'] . "', OrderDate = '" .$Config['TodayDate']. "', SubTotalPrice = '" . $SubTotalPrice . "', TotalPrice = '" . $TotalPrice . "', 
                            TotalQuantity =  '" . $TotalQuantity . "', Tax =  '" . $Tax . "', Shipping =  '" . $Shipping . "', ShippingMethod =  '" . $ShippingMethod . "', Weight =  '".$Weight. "', WeightUnit = '" .$WeightUnit. "',
                            ...
                            ShippingZip = '" . $ShippingZip . "', ShippingPhone = '" . $ShippingPhone . "', ShippingAddressType = '".$AddressType."', payment_gateway = '" . $payment_gateway . "',  DelivaryDate = '" . $DelivaryDate . "', DiscountVoucher = '" . addslashes($DiscountVoucher) . "', Discount = '" . addslashes($DiscountAmount) . "'";
        ...
        for ($i = 0; $i < sizeof($arryRow); $i++) {
            ...
            $Price = $Config['CurrencyValue'] * $arryRow[$i]['Price'];
    ...
    function RemoveOrder($id) {
        $strSQLQuery = "delete from e_orders where OrderID=" . $id;
        $this->query($strSQLQuery, 0);
        $strSQLQuery = "delete from e_orderdetail where OrderID=" . $id;
        $this->query($strSQLQuery, 0);
        return 1;
    }
    ...
    function PaymentDone($Cid, $OrderID, $PaymentGateway) {
        ...
        if (sizeof($arryRow) > 0) {
            ...
            if (!empty($payment_gateway)) {
                $sqlPay = "update e_orders set OrderStatus = 'Process', ShippingStatus = 'Pending', PaymentStatus=1, payment_gateway='" . $payment_gateway . "' where OrderID=".$OrderID;
                $this->query($sqlPay, 0);
            }
            ...
            <td align="right" colspan="5">Sub Total:  ' . display_price($SubTotal, '', '', '', '') . '</td>
                ...
                if ($arryRow[0]['Discount'] > 0) {
                    $ProductsDet .='<tr>
                            <td align="right" colspan="5">Discount (Voucher Code:' . $arryRow[0]['DiscountVoucher'] . '):  - ' . display_price($arryRow[0]['Discount'], '', '', '', '') . ' </td>
                                </tr>';
                }

                $sTotal = $SubTotal;
                $Total = $sTotal + $shipPrice + $Tax - $arryRow[0]['Discount'];
    ...
    function OrderDeliver($OrderID) {
        ...
        if (is_array($arryProducts) && $numProducts > 0) {
         ...
         <td height="25" colspan="2" align="left" valign="top"  ><SPAN >' . $arryProducts[$i]['ProductName'] . '</SPAN></td><td height="25" align="center" valign="top"  >' . $arryProducts[$i]['Quantity'] . '</td><td height="25" align="center" valign="top"  >&pound;' . number_format($arryProducts[$i]['Price'], 2, '.', '') . '</td><td height="25" align="center" valign="top"  >&pound;' . number_format($arryProducts[$i]['Quantity'] * $arryProducts[$i]['Price'], 2, '.', '') . '</td></tr>';
            ...
            $Tax = $arryRow[0]['Tax'] * $SubTotal / 100;
            $shipPrice = $arryRow[0]['Shipping'] * $SubTotal / 100;
            $sTotal = $SubTotal;
            $Total = number_format(($sTotal + $shipPrice + $Tax), 2, '.', '');

            $ProductsDet .='<tr><td colspan="2" align="right"  ><table width="98%" border="0" cellpadding="2" cellspacing="1"><tr><td align="right"  >Shipping   Charges :  $' . number_format($shipPrice, 2, '.', '') . '</td></tr></table></td><td height="30" align="right"  class="redlink">&nbsp;</td><td height="30" align="right"  colspan="2" >Sub Total   :  $' . number_format($SubTotal, 2, '.', '') . '</td></tr>';
            $ProductsDet .='<tr><td colspan="2" align="right"  >Tax   :  $' . number_format($Tax, 2, '.', '') . '&nbsp;</td><td height="30" colspan="3" align="right"  >';
        }  //echo $ProductsDet;
        //exit;

        $cont = "Your order has been dispatched successfully !!";
        $contents = file_get_contents("../html/delOrder.htm");
        ...
        $contents = str_replace("[Phone]", $arryRow[0]['Phone'], $contents);
        $contents = str_replace("[Email]", $arryRow[0]['Email'], $contents);
        $contents = str_replace("[Billing Name]", $arryRow[0]['BillingFName'] . " " . $arryRow[0]['BillingLName'], $contents);
        $contents = str_replace("[Billing Add]", $arryRow[0]['BillingStreet'], $contents);
        $contents = str_replace("[Billing City]", $arryRow[0]['BillingCity'], $contents);
        $contents = str_replace("[Billing State]", $arryRow[0]['BillingState'], $contents);
        $contents = str_replace("[Billing Country]", $arryRow[0]['BillingCountry'], $contents);
        $contents = str_replace("[Billing Zip]", $arryRow[0]['BillingZip'], $contents);
        $contents = str_replace("[Shipping Name]", $arryRow[0]['ShippingFName'] . " " . $arryRow[0]['ShippingLName'], $contents);
        $contents = str_replace("[Shipping Add]", $arryRow[0]['ShippingStreet'], $contents);
        $contents = str_replace("[Shipping City]", $arryRow[0]['ShippingCity'], $contents);
        $contents = str_replace("[Shipping Country]", $arryRow[0]['ShippingCountry'], $contents);
        $contents = str_replace("[Shipping State]", $arryRow[0]['ShippingState'], $contents);
        $contents = str_replace("[Shipping Zip]", $arryRow[0]['ShippingZip'], $contents);
    ...
    function changePaymentStatus($id, $stat) {
        $sql = "update e_orders set PaymentStatus=" . $stat . " where OrderID=" . $id;
        $this->query($sql, 0);
    }
    ...
    function changeStatus($id, $stat) {
        $sql = "update e_orders set where OrderID=" . $id;
        $this->query($sql, 0);
        /* $arrRows=$this->getOrderDet($id);
          if($stat==1){
          $sql1="update painting set sellStatus='Sold' where paintingID in (".$arrRows[0]['ProductIDs'].")";

          }
          else{
          $sql1="update painting set sellStatus='Stock' where paintingID in (".$arrRows[0]['ProductIDs'].")";

          } */

        if ($stat == 1) {
            //$this->OrderDeliver($id);
        }
        return true;
    }
