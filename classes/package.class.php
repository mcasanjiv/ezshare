...
class package extends dbClass
{
		...
		function SendPackagePrdEmail($ProductID,$PackageID,$PaymentGateway)
		{
			...
			$sql ="select p.*,m.UserName,m.Email,m.MemberID from products p inner join members m on p.PostedByID=m.MemberID where p.ProductID = '".$ProductID."' and p.Featured!='Yes'";
			...
			/******************/
			$objPay=new payment();
			$objPay->addPayment(stripslashes($arryProduct[0]['Name']).' - '.$PaymentFor,$ProductID.','.$PackageID , '6', $arryProduct[0]['MemberID'], $arryProduct[0]['UserName'], $PaymentGateway, $arryPackage[0]['Price'], $PaymentStatus);
			/******************/
				...
				if($PaymentGateway==3 || $PaymentGateway==4){
					$UserMesg = '<br><br>Your request to make the product , ['.stripslashes($arryProduct[0]['Name']).'], as featured has been received <br>and it will be processed once the amount due has reflected in our bank statements.<br> Please check your registered email address for further details.';
					$subject = 'Your request to make the product , ['.stripslashes($arryProduct[0]['Name']).'], as featured has been received';
				}else{
					$UserMesg = 'Your product, '.stripslashes($arryProduct[0]['Name']).', has been marked as featured.';
					$subject = $UserMesg;
		...
		function UpdateFeaturedPrdAdmin($ProductID,$PackageID,$PaymentGateway)
		{
		...
		function SendPackageStoreEmail($MemberID,$PackageID,$PaymentGateway)
		{
			...
			/******************/
			$objPay=new payment();
			$objPay->addPayment(stripslashes($arryMember[0]['CompanyName']).'-'.$PaymentFor,$PackageID , '4', $MemberID, $arryMember[0]['UserName'], $PaymentGateway, $arryPackage[0]['Price'], $PaymentStatus);
			/******************/
				...
				$contents = str_replace("[ADMIN_LINK]",$Config['Url']."admin/editSeller.php?opt=Seller&edit=".$MemberID,$contents);
				...
				$mail->sender($Config['SiteName'], $Config['AdminEmail']);   
				$mail->Subject = $Config['SiteName']." - Featured store payment has been processed";
				$mail->IsHTML(true);
	...
	function SendPackageWebsiteEmail($MemberID,$PackageID,$PaymentGateway)
		{
			$arryPackage = $this->getPackage($PackageID,'');
			...
			if($arryPackage[0]['PackageID']>0 && $arryMember[0]['MemberID']>0 ){
				...
				if($PaymentStatus==1){
					$strSQLQuery = "update members set FeaturedWeb='Yes',FeaturedWebAmount='".$arryPackage[0]['Price']."',WebImpression='".$arryPackage[0]['Impression']."',WebImpressionCount=0,FeaturedWebStart='".$FeaturedStart."',FeaturedWebEnd='".$FeaturedEnd."',FeaturedWebType='".$arryPackage[0]['Type']."' where MemberID=".$MemberID; 
					$this->query($strSQLQuery, 0);
				}

			/******************/
			$objPay=new payment();
			$objPay->addPayment(stripslashes($arryMember[0]['CompanyName']).'-'.$PaymentFor,$PackageID , '8', $MemberID, $arryMember[0]['UserName'], $PaymentGateway, $arryPackage[0]['Price'], $PaymentStatus);
			/******************/

				global $Config;

				$AdminMesg = $arryMember[0]['UserName'].' has done payment to make his website as featured  on <a href="'.$Config['Url'].'" target="_blank" class="normallink">'.$Config['SiteName'].'</a>.';

				$contents = file_get_contents("html/featuredWebAdmin.htm");
				...
				$StoreUrl = $Config['Url'].$Config['StorePrefix'].$arryMember[0]['UserName'].'/home.php';
				...
				$contents = str_replace("[ADMIN_LINK]",$Config['Url']."admin/editSeller.php?edit=".$MemberID,$contents);
				
				$contents = str_replace("[STORE_LINK]",$Config['Url'].$Config['StorePrefix'].$arryMember[0]['UserName']."/home.php",$contents);
				...
				$mail->AddAddress($Config['AdminEmail']);
				$mail->sender($arryMember[0]['UserName'], $arryMember[0]['Email']);   
				$mail->Subject = $Config['SiteName']." - Featured website payment";
				$mail->IsHTML(true);
				...
				$contents = file_get_contents("html/featuredWebsite.htm");

				$UserMesg = '<BR>
				Your payment to have your website featured on <a href="'.$Config['Url'].'" target="_blank" class="normallink">'.$Config['SiteName'].'</a> has been processed successfully.';
				...
				$mail->AddAddress($arryMember[0]['Email']);
				$mail->sender($Config['SiteName'], $Config['AdminEmail']);   
				$mail->Subject = $Config['SiteName']." - Featured website payment has been processed";
				$mail->IsHTML(true);
		...
		function SendPackageCreditEmail($MemberID,$PackageID,$PaymentGateway,$CreditType)
		{
			$arryPackage = $this->getPackage($PackageID,'');

			 $sql ="select m.MemberID,m.UserName,m.Email,m.CompanyName,MaxEmail,MaxSms from members m where m.MemberID = '".$MemberID."' ";
			$arryMember = $this->query($sql, 1);
			...
			if($PaymentGateway==3 || $PaymentGateway==4){
				...
				$strSQLQuery22 ="select * from configuration where ConfigID=1";
				$arryRow22 = $this->query($strSQLQuery22, 1);
					$payment_method_detail .= '<table border=0 cellpadding=0 cellspacing=3>';
					$payment_method_detail .= '<tr><td nowrap>Account Holder : </td><td>'.$arryRow22[0]['AccountHolder'].'</td></tr>';
					$payment_method_detail .= '<tr><td nowrap>Account Number : </td><td>'.$arryRow22[0]['AccountNumber'].'</td></tr>';
					$payment_method_detail .= '<tr><td nowrap>Bank Name : </td><td>'.$arryRow22[0]['BankName'].'</td></tr>';
					$payment_method_detail .= '<tr><td nowrap>Branch Code : </td><td>'.$arryRow22[0]['BranchCode'].'</td></tr>';
			...
			if($arryPackage[0]['PackageID']>0 && $arryMember[0]['MemberID']>0 ){
				...
				if($CreditType=='sms'){
					$PaymentFor = $arryPackage[0]['Impression'].' SMS Credits';
					$CatID = 10;
					$MaxSms = $arryPackage[0]['Impression']+$arryMember[0]['MaxSms'];

					 $strSQLQuery = "update members set MaxSms='".$MaxSms."' where MemberID=".$MemberID; 
				}else{
					$PaymentFor = $arryPackage[0]['Impression'].' Email Credits';
					$CatID = 9;
					$MaxEmail = $arryPackage[0]['Impression']+$arryMember[0]['MaxEmail'];

					$strSQLQuery = "update members set MaxEmail='".$MaxEmail."' where MemberID=".$MemberID; 
			...
			/******************/
			$objPay=new payment();
			$objPay->addPayment($PaymentFor,$CreditType.','.$PackageID , $CatID, $MemberID, $arryMember[0]['UserName'], $PaymentGateway, $arryPackage[0]['Price'], $PaymentStatus);
			/******************/

				global $Config;

				$AdminMesg = $arryMember[0]['UserName'].' has done payment for '.$PaymentFor.'  on <a href="'.$Config['Url'].'" target="_blank" class="normallink">'.$Config['SiteName'].'</a>.
				<br><br>Amount: '.$Config['Currency'].' '.$arryPackage[0]['Price'].'
				<br><br>Payment Method: '.$payment_gateway.'<br><br>';
				...
				$mail->AddAddress($Config['AdminEmail']);
				$mail->sender($arryMember[0]['UserName'], $arryMember[0]['Email']);   
				$mail->Subject = $Config['SiteName']." - Payment for ".$PaymentFor;
				$mail->IsHTML(true);
				...
				$UserMesg = 'Dear '.$arryMember[0]['UserName'].',<br><br>'.date("jS, F Y").'<BR><BR>
				Your payment to have '.$PaymentFor.' on <a href="'.$Config['Url'].'" target="_blank" class="normallink">'.$Config['SiteName'].'</a> has been processed successfully.';

				if($PaymentGateway==3 || $PaymentGateway==4){
					$UserMesg .= '<br><br>Your request will be processed once the amount due has reflected in our bank statements.<br> Please check your registered email address for further details.';
				}

				$UserMesg .= '<br><br>Amount: '.$Config['Currency'].' '.$arryPackage[0]['Price'].'
				<br><br>Payment Method: '.$payment_gateway.'<br><br>'.$payment_method_detail;
				...
				$mail->AddAddress($arryMember[0]['Email']);
				$mail->sender($Config['SiteName'], $Config['AdminEmail']);   
				$mail->Subject = $Config['SiteName']." - Payment for ".$PaymentFor." has been processed";
				$mail->IsHTML(true);
		...
		function UpdateCreditAdmin($MemberID,$PackageID,$PaymentGateway,$CreditType)
		{
			$arryPackage = $this->getPackage($PackageID,'');
			...
			if($arryPackage[0]['PackageID']>0 && $arryMember[0]['MemberID']>0 ){
				...
				$UserMesg = 'Dear '.$arryMember[0]['UserName'].',<br><br>'.date("jS, F Y").'<BR><br><BR>
				Your payment to have '.$PaymentFor.' on <a href="'.$Config['Url'].'" target="_blank" class="normallink">'.$Config['SiteName'].'</a> has been received successfully.<br><br>'.$PaymentFor.' has been updated into your account.' ;
		...
		function UpdateFeaturedStoreAdmin($MemberID,$PackageID,$PaymentGateway)
		{
			$arryPackage = $this->getPackage($PackageID,'');
		...
		function UpdateFeaturedWebsiteAdmin($MemberID,$PackageID,$PaymentGateway)
		{
			$arryPackage = $this->getPackage($PackageID,'');
			...
			if($arryPackage[0]['PackageID']>0 && $arryMember[0]['MemberID']>0 ){
				...
				$UserMesg = '<BR>
				Your payment to have your website featured on <a href="'.$Config['Url'].'" target="_blank" class="normallink">'.$Config['SiteName'].'</a> has been received successfully.<br><br>your website has been marked as featured.';
				...
				$mail->AddAddress($arryMember[0]['Email']);
				$mail->sender($Config['SiteName'], $Config['AdminEmail']);   
				$mail->Subject = $Config['SiteName']." - your website has been marked as featured";
				$mail->IsHTML(true);
		...
		function UpdateSponsoredPrdAdmin($ProductID,$PackageID,$PaymentGateway)
		{

			$arryPackage = $this->getPackage($PackageID,'');

			$sql ="select p.*,m.UserName,m.Email,m.MemberID,m.CompanyName from products p inner join members m on p.PostedByID=m.MemberID where p.ProductID = '".$ProductID."' and p.Sponser!='Yes'";
			...
			if($arryPackage[0]['PackageID']>0 && $arryProduct[0]['ProductID']>0 ){
				...
				$UserMesg = 'Your product, '.stripslashes($arryProduct[0]['Name']).', has been marked as sponsored.';
				$subject = $UserMesg;
	...
	function SendPackageSpnEmail($ProductID,$PackageID,$PaymentGateway)
		{
			$arryPackage = $this->getPackage($PackageID,'');

			$sql ="select p.*,m.UserName,m.Email,m.MemberID from products p inner join members m on p.PostedByID=m.MemberID where p.ProductID = '".$ProductID."' and p.Sponser!='Yes'";
			$arryProduct = $this->query($sql, 1);
			...
			/******************/
			$objPay=new payment();
			$objPay->addPayment(stripslashes($arryProduct[0]['Name']).' - '.$PaymentFor,$ProductID.','.$PackageID , '7', $arryProduct[0]['MemberID'], $arryProduct[0]['UserName'], $PaymentGateway, $arryPackage[0]['Price'], $PaymentStatus);
			/******************/
				...
				$AdminMesg = $arryProduct[0]['UserName'].' has done payment to make his below product as sponsored  on <a href="'.$Config['Url'].'" target="_blank" class="normallink">'.$Config['SiteName'].'</a>.';

				$contents = file_get_contents("html/sponserPrdAdmin.htm");
				...
				if($PaymentGateway==3 || $PaymentGateway==4){
					$UserMesg = '<br><br>Your request to make the product , ['.stripslashes($arryProduct[0]['Name']).'], as sponsored has been received <br>and it will be processed once the amount due has reflected in our bank statements.<br> Please check your registered email address for further details.';
					$subject = 'Your request to make the product , ['.stripslashes($arryProduct[0]['Name']).'], as sponsored has been received';
				}else{
					$UserMesg = 'Your product, '.stripslashes($arryProduct[0]['Name']).', has been marked as sponsored.';
					$subject = $UserMesg;
	...
	function SendPackageBannerEmail($BannerID,$PackageID,$PaymentGateway)
		{
			...
			$sql ="select b.*,m.UserName,m.Email from banner b inner join members m on b.MemberID=m.MemberID where b.BannerID = '".$BannerID."'";
			...
			if($PaymentGateway==3 || $PaymentGateway==4){
				...
				/******************/
				$objPay=new payment();
				$objPay->addPayment(stripslashes($arryBanner[0]['Title']).' - '.$PaymentFor,$BannerID,'3',$arryBanner[0]['MemberID'], $arryBanner[0]['UserName'], $PaymentGateway, $arryPackage[0]['Price'], $PaymentStatus);
				/******************/
				...
				$UserMesg = 'Dear '.$arryBanner[0]['UserName'].',<BR><BR>'.date("jS, F Y").'<BR><BR>
				Your payment for the banner below has been successfully processed.';
				...
				if (empty($arryRow[0]['FeaturedID'])) {
					...
					if($FeatType=='Impression'){
						if($Type=='Product') {
							...
							$strSQLQuery = "update members set WebImpressionCount=WebImpressionCount+1 where MemberID=".$FeaturedID; 
						}
