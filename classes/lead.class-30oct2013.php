...
function isOpportunityNameExists($OpportunityName,$OpportunityID=0)
	...
	function  ListTicket($id=0,$parent_type,$parentID,$SearchKey,$SortBy,$AscDesc)
		{
			$strAddQuery = '';
			$SearchKey   = strtolower(trim($SearchKey));
		...
		function GetCommentUser($id,$parentID,$parent_type,$parent,$status=0){
         ...
         $strAddQuery = 'where 1';
		  $strAddQuery .= (!empty($id))?(" c.CommentID=".$id):("  ");
			...
			$strAddQuery .= " order by c.CommentDate asc ";
...
//echo $strSQLQuery;exit;
		...
		function  ListCampaign($id=0,$SearchKey,$SortBy,$AscDesc)
		{
			$strAddQuery = '';
			$SearchKey   = strtolower(trim($SearchKey));
...
//echo"<pre>";
			...
			//if(empty($Status)) $Status=1;
            echo $strSQLQuery = "update c_campaign set  campaignname='".addslashes($campaignname)."',
