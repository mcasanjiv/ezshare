...
{ 
	...
	function addActivity($arryDetails)
	{
		...
		$arryAsign = explode(":",$assignedTo);
		 ...
		 $sql = "insert into c_activity  (subject, title, created_by, created_id, description, startDate, startTime, closeDate, closeTime, status, activityType, location, priority, visibility, Notification, reminder, parent_type, parentID, assignedTo, AssignType, GroupID, RelatedType, LeadID, OpprtunityID, CampaignID, TicketID, QuoteID, add_date, CustID, EntryType,EntryFrom,EntryTo,EntryDate,EntryInterval,EntryMonth) values( '".addslashes($subject)."', '".addslashes($title)."','".addslashes($created_by)."','".addslashes($created_id)."','".addslashes($description)."', '".$startDate."','".$startTime."','".$closeDate."','".$closeTime."','".$status."','".$activity_type."', '".$location."', '".addslashes($priority)."', '".addslashes($visibility)."','".$Notification."','".$reminder."','".$parent_type."','".$parentID."','".$AssignUser."','".$AssignType."','".$GroupID."','".$RelatedType."','".$LeadID."','".$OpprtunityID."', '".$CampaignID."', '".$TicketID."', '".$QuoteID."','".$Config['TodayDate']."' , '".$CustID."' , '".$EntryType."', '".$EntryFrom."', '".$EntryTo."', '".$EntryDate."','".$EntryInterval."','".$EntryMonth."')"; 
	...
	function updateActivity($arryDetails)
	{
		@extract($arryDetails);	
			  ...
			  $GroupID =  $arryAsign[1];
                ...
                if($EntryType == 'one_time'){$EntryDate=0;$EntryFrom='';$EntryTo='';$EntryInterval='';$EntryMonth='';} 
                if($EntryInterval == 'monthly'){$EntryMonth='';}
...
$strAddQuery .= ($Config['vAllRecord']!=1)?(" and (FIND_IN_SET(".$_SESSION['AdminID'].",e.assignedTo) OR e.created_id='".$_SESSION['AdminID']."') "):("");
	...
	function  GetActivityList($arryDetails)
	{
		...
		if($st==1){
			...
			$sortby=' e.startDate,e.startTime, e.closeDate'; $asc='asc';
		}
