...
/****************Sales/Purchases Function Start******************/	
					...
					$strAddQuery .= (!empty($asc))?($asc):(" desc");
                                        ...
                                        $strSQLQuery = "select (SELECT SUM(p.CreditAmnt) FROM f_payments p WHERE p.InvoiceID = o.InvoiceID) AS paidAmnt,o.OrderType, o.OrderDate, o.PostedDate, o.OrderID, o.PurchaseID, o.QuoteID, o.InvoiceID, o.SuppCode, o.SuppCompany, o.TotalAmount, o.Currency,o.InvoicePaid,o.InvoiceEntry  from p_order o ".$strAddQuery;
		...
		{
		    $strSQLQuery = "select sum(DebitAmnt) as total from f_payments where InvoiceID='".$InvoiceID."' and PaymentType = 'Sales'";
			$arryRow = $this->query($strSQLQuery, 1);
		...
		{
		    $strSQLQuery = "select sum(DebitAmnt) as total from f_payments where SaleID='".$SaleID."' and PaymentType = 'Sales'";
			$arryRow = $this->query($strSQLQuery, 1);
			return $arryRow[0]['total'];
		...
		{
		    $strSQLQuery = "select sum(CreditAmnt) as total from f_payments where InvoiceID='".$InvoiceID."' and PaymentType='Purchase'";
			//echo "=>".$strSQLQuery;
		    $arryRow = $this->query($strSQLQuery, 1);
			return $arryRow[0]['total'];
		...
		{
		    $strSQLQuery = "select sum(CreditAmnt) as total from f_payments where PurchaseID='".$PurchaseID."' AND PaymentType='Purchase'";
                    //echo "=>".$strSQLQuery;exit;
		    $arryRow = $this->query($strSQLQuery, 1);
		   return $arryRow[0]['total'];
			...
			$ipaddress = $_SERVER["REMOTE_ADDR"];
                        ...
                        for($i=1;$i<=$totalInvoice;$i++){
                          if($arryDetails['invoice_check_'.$i] == 'on' && $arryDetails['payment_amnt_'.$i] > 0){
                                ...
                                $strSQLQueryPay = "INSERT INTO f_payments SET PID='".$PID."', DebitAmnt = '".$arryDetails['payment_amnt_'.$i]."', AccountID = '32', ExpenseID = '".$ExpenseID."', SuppCode = '".$SuppCode."', ReferenceNo = '".addslashes($ReferenceNo)."', Method= '".addslashes($Method)."', CheckBankName='".addslashes($CheckBankName)."', CheckFormat='".$CheckFormat."', PaymentDate = '".$Date."', Comment = '".addslashes($Comment)."', Currency = '". $Config['Currency']."', LocationID='".$_SESSION['locationID']."', PaymentType='Purchase', Flag= 1, IPAddress='".$ipaddress."'";
                                $this->query($strSQLQueryPay, 0);
		...
		function updatePurchaseInvoiceStatus($InvoiceID,$chk)
			...
			function addOtherExpense($arryDetails)
			{
				...
				$strSQLQuery = "INSERT INTO f_expense SET  InvoiceID  = '".$InvoiceID."', OrderID='".$OrderID."', Amount = '".$Amount."', TaxID = '".$TaxID."',TaxRate='".$TaxRate."', TotalAmount = '".$TotalAmount."', BankAccount = '".$BankAccount."', PaidTo = '".$PaidTo."', ReferenceNo = '".addslashes($ReferenceNo)."', PaymentMethod= '".addslashes($PaymentMethod)."', PaymentDate = '".$PaymentDate."', Comment = '".addslashes($Comment)."', Currency = '".$Config['Currency']."', LocationID='".$_SESSION['locationID']."', ExpenseTypeID='".$ExpenseTypeID."',CreatedDate='".$Config['TodayDate']."',IPAddress='".$ipaddress."',EntryType='".$EntryType."', EntryInterval='".$EntryInterval."',EntryMonth='".$EntryMonth."', EntryWeekly = '".$EntryWeekly."', EntryFrom='".$EntryFrom."',EntryTo='".$EntryTo."',EntryDate='".$EntryDate."',GlEntryType='".$GlEntryType."'";
				$this->query($strSQLQuery, 0);	
				$ExpenseID = $this->lastInsertId();		

				/*********Add Payment Transaction**********/
                                
				$strSQLQueryPay = "INSERT INTO f_payments SET  InvoiceID  = '".$InvoiceID."', OrderID='".$OrderID."', PurchaseID='".addslashes($ReferenceNo)."', CreditAmnt = '".$TotalAmount."', AccountID = '".$BankAccount."', ExpenseID = '".$ExpenseID."', SuppCode = '".$PaidTo."', ReferenceNo = '".addslashes($ReferenceNo)."', Method= '".addslashes($PaymentMethod)."', PaymentDate = '".$PaymentDate."', Comment = '".addslashes($Comment)."', Currency = '". $Config['Currency']."', LocationID='".$_SESSION['locationID']."', PaymentType='Spiff Expense',IPAddress='".$ipaddress."'";
				$this->query($strSQLQueryPay, 0);
                                $PID = $this->lastInsertId();
                                
                                if($GlEntryType == "Single"){
                                    $strSQLQueryPay = "INSERT INTO f_payments SET  PID='".$PID."',  DebitAmnt = '".$TotalAmount."', AccountID = '".$ExpenseTypeID."', ExpenseID = '".$ExpenseID."', SuppCode = '".$PaidTo."', ReferenceNo = '".addslashes($ReferenceNo)."', Method= '".addslashes($PaymentMethod)."', PaymentDate = '".$PaymentDate."', Comment = '".addslashes($Comment)."', Currency = '". $Config['Currency']."', LocationID='".$_SESSION['locationID']."', PaymentType='Spiff Expense', Flag= 1, IPAddress='".$ipaddress."'";
                                    $this->query($strSQLQueryPay, 0);
                                }else{
                                    
                                    for($i=1;$i<=$NumLine1;$i++){
                                        if($arryDetails['GlAmnt'.$i] > 0 && !empty($arryDetails['AccountID'.$i])){
                                            $strSQLQueryPayMul = "INSERT INTO f_payments SET  PID='".$PID."',  DebitAmnt = '".$arryDetails['GlAmnt'.$i]."', AccountID = '".$arryDetails['AccountID'.$i]."', ExpenseID = '".$ExpenseID."', SuppCode = '".$PaidTo."', ReferenceNo = '".addslashes($ReferenceNo)."', Method= '".addslashes($PaymentMethod)."', PaymentDate = '".$PaymentDate."', Comment = '".addslashes($Comment)."', Currency = '". $Config['Currency']."', LocationID='".$_SESSION['locationID']."', PaymentType='Other Expense', Flag= 1, IPAddress='".$ipaddress."'";
                                            $this->query($strSQLQueryPayMul, 0);

                                            $strSQLQueryPayMulAcc = "INSERT INTO f_multi_account_payment SET  AccountID='".$arryDetails['AccountID'.$i]."', AccountName = '".$arryDetails['AccountName'.$i]."', Amount = '".$arryDetails['GlAmnt'.$i]."',  ExpenseID = '".$ExpenseID."'";
...
/**************************End Account Balance************************************************************/
            ...
            function isInvoiceNumberExistsForIncome($InvoiceID)
		{
			...
			if (!empty($arryRow[0]['IncomeID'])) {
                ...
                {
                       $strSQLQuery = "SELECT * from f_multi_account_payment where ExpenseID = '".trim($ExpenseID)."'";
