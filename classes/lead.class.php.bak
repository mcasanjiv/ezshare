...
class lead extends dbClass
{
		...
		function  ListLead($id=0,$SearchKey,$SortBy,$AscDesc)
		{
			...
			$strAddQuery .= ($_SESSION['AdminType']!="admin")?(" and (l.AssignTo='".$_SESSION['AdminID']."' OR l.created_id='".$_SESSION['AdminID']."')"):("");
	...
	function  ListTicket($PID,$parent_type,$parentID,$SearchKey,$SortBy,$AscDesc)
		{
			...
			}*/          if($SearchKey != "" && $SortBy == "t.TicketID"){$strAddQuery .= " where t.TicketID = '".$SearchKey."'";	
                        }  elseif($SearchKey != "" && $SortBy == "t.title"){$strAddQuery .= " where t.title like '%".$SearchKey."%'";			
                        }
                        elseif($SortBy=='t.Status')
                        {   
                                  $strAddQuery .= " where t.Status='".$SearchKey."'";
                        }
      ...
      function  ListDocument($id=0,$parent_type,$parentID,$SearchKey,$SortBy,$AscDesc)
		{
			...
			$strAddQuery .= ($_SESSION['AdminType']!="admin")?(" and d.AssignTo='".$_SESSION['AdminID']."' "):("");
...
//echo $strSQLQuery;exit;
		...
		function  ListOpportunity($id=0,$SearchKey,$SortBy,$AscDesc)
		{
			...
			$strAddQuery .= ($_SESSION['AdminType']!="admin")?(" and o.AssignTo='".$_SESSION['AdminID']."' "):("");
			...
			$strAddQuery .= (!empty($AscDesc))?($AscDesc):(" desc");
			 ...
			 $strSQLQuery = "select o.OpportunityID,o.LeadID,o.Status,o.OpportunityName,o.lead_source,o.CloseDate,o.SalesStage,o.description,e.EmpID,d.Department,e.Role,e.UserName as AssignTo from c_opportunity o left outer join  h_employee e on e.EmpID=o.AssignTo left outer join  h_department d on e.Department=d.depID ".$strAddQuery;
...
//echo"<pre>";
			...
			$htmlPrefix = $Config['EmailTemplateFolder'];
			$contents = file_get_contents($htmlPrefix."com_assigned.htm");
			$subject  = "  Assigned to You ";
			$CompanyUrl = $Config['Url'].$_SESSION['DisplayName'].'/admin/';
			$contents = str_replace("[URL]",$Config['Url'],$contents);
			$contents = str_replace("[SITENAME]",$Config['SiteName'],$contents);
			...
			$contents = str_replace("[TARGET]",stripslashes($targetaudience), $contents);
