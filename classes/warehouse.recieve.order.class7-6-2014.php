...
class wrecieve extends dbClass
{
		...
		function  SalesReport($FilterBy,$FromDate,$ToDate,$Month,$Year,$CustCode,$SalesPID,$Status)
		{
			...
			$strSQLQuery = "select o.OrderDate, o.PostedDate, o.OrderID, o.SaleID, o.CustCode, o.CustomerName, o.SalesPerson, o.CustomerCurrency, o.Freight, o.discountAmnt, o.taxAmnt, o.TotalAmount, o.Status,o.Approved  from w_order_recieve  o where o.Module='Order' ".$strAddQuery." order by o.OrderDate desc";
				//echo "=>".$strSQLQuery;
			return $this->query($strSQLQuery, 1);		
		}
		...
		function  GetNumSOByYear($Year,$FromDate,$ToDate,$CustCode,$SalesPID,$Status)
		{
			...
			$strSQLQuery = "select count(o.OrderID) as TotalOrder  from w_order_recieve  o where o.Module='Order' and YEAR(o.OrderDate)='".$Year."' ".$strAddQuery." order by o.OrderDate desc";
		...
		function  GetOrderAmountByMonth($Year,$FromDate,$ToDate,$CustCode,$Status)
		{
			...
			$strSQLQuery = "select o.TotalAmount,o.CustomerCurrency from w_order_recieve  o where o.Module='Order' and YEAR(o.OrderDate)='".$Year."' ".$strAddQuery." order by o.OrderDate desc";
		...
		function getCustomerOrderedAmount($FilterBy,$FromDate,$ToDate,$Month,$Year,$CustCode,$Status)
		{
			...
			$strSQLQuery = "select SUM(TotalAmount) as totalOrderAmnt from w_order_recieve  as o WHERE o.Module='Order' ".$strAddQuery;
			//echo $strSQLQuery;exit;
			$rs = $this->query($strSQLQuery, 1);
		...
		function UpdateInvoice($arryDetails){ 
			...
			$strSQLQuery = "update w_order_recieve  set ShippedDate='".$ShippedDate."', PaymentDate='".$PaymentDate."', InvoicePaid='".$InvoicePaid."', InvPaymentMethod='".$InvPaymentMethod."', PaymentRef='".addslashes($PaymentRef)."', InvoiceComment='".addslashes($InvoiceComment)."', UpdatedDate = '".$Config['TodayDate']."'
		...
		{
			$strSQLQuery = "select count(o.OrderID) as TotalInvoice from w_order_recieve  o where o.Module='Invoice' and SaleID='".$SaleID."'";
		...
		function updateInvoiceStatus($oid)
		{
		   $strSQLQuery = "update w_order_recieve  set InvoicePaid = 'Paid' where OrderID='".$oid."'";
		   $this->query($strSQLQuery, 0);
		}
		
		function updateOrderStatus($SaleID)
		{
		   $strSQLQuery = "update w_order_recieve  set Status = 'Completed' where SaleID='".$SaleID."'";
		   $this->query($strSQLQuery, 0);
		}
		...
		function isEmailExists($Email,$OrderID=0)
		{
			...
			$strSQLQuery = "select OrderID from w_order_recieve  where LCASE(Email)='".strtolower(trim($Email))."'".$strSQLQuery;
			$arryRow = $this->query($strSQLQuery, 1);
		...
		{
			$strSQLQuery = (!empty($OrderID))?(" and OrderID != ".$OrderID):("");
			$strSQLQuery = "select OrderID from w_order_recieve  where Module='Quote' and QuoteID='".trim($QuoteID)."'".$strSQLQuery;
		...
		{
			$strSQLQuery = (!empty($OrderID))?(" AND OrderID != '".$OrderID."'"):("");
			$strSQLQuery = "SELECT OrderID from w_order_recieve  where Module='Invoice' AND InvoiceID = '".trim($InvoiceID)."'".$strSQLQuery;
...
function  ListTransferRecieve($arryDetails)
		{
			...
			$strAddQuery .= (!empty($asc))?($asc):(" desc");
			 ...
			 $strSQLQuery = "select o.transferDate,o.refID,o.transferNo,o.transferID,o.total_transfer_value,o.to_WID,o.from_WID,t.warehouse_name as to_warehouse,t.WID,t.warehouse_code as to_warehouse,f.warehouse_name as from_warehouse,f.WID,f.warehouse_code as fr_warehouse,  o.transferID,r.RecieveID,r.RecievedDate,r.transaction_ref from w_transfer  o left outer join w_warehouse t on  t.WID =  o.to_WID left outer join w_warehouse  f on  f.WID =  o.from_WID left outer join w_transfer_recieve  r on  r.OrderID = o.transferID ".$strAddQuery;
		//echo $strSQLQuery;
			return $this->query($strSQLQuery, 1);		
