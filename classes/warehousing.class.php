...
class common extends dbClass
{
		...
		{
			@extract($arryAtt);	
			$sql = "update w_attribute_value set attribute_value = '".addslashes($attribute_value)."',attribute_id = '".$attribute_id."',Status = '".$Status."'  where value_id = ".$value_id; 
			$rs = $this->query($sql,0);
		...
		{
			$sql = "delete from w_attribute_value where value_id = ".$id;
			$rs = $this->query($sql,0);
