...
class contact extends dbClass
{
		...
		function  ListContact($id=0,$SearchKey,$SortBy,$AscDesc)
		{
			...
			$strAddQuery .= (!empty($SortBy))?(" order by ".$SortBy." "):(" order by c.ContactID ");
			$strAddQuery .= (!empty($AscDesc))?($AscDesc):(" Asc");

			$strSQLQuery = "select c.ContactID,c.Status,c.LastName,c.title,c.Email,c.FirstName,c.Image,d.Department,e.Role,e.UserName as AssignTo from c_contact c left outer join  h_employee e on e.EmpID=c.AssignTo left outer join  department d on e.Department=d.depID ".$strAddQuery;
		...
		{  
			...
			$ipaddress = $_SERVER["REMOTE_ADDR"];
			 ...
			 $strSQLQuery = "insert into c_contact (LeadID,locationID,Email,FirstName,LastName,date_of_birth,Address, city_id, state_id, ZipCode, country_id,Mobile, LandlineNumber, PersonalEmail,  Status, OtherState, OtherCity,title, lead_source, Organization,ReportsTo,AssignTo, reference, Department, do_not_call, notify_owner, email_opt_out, PortalUser, Supp_start_date,Supp_end_date,ipaddress ,description, UpdatedDate ) values( '".$LeadID."','".$_SESSION['locationID']."', '".addslashes($Email)."', '".addslashes($FirstName)."', '".addslashes($LastName)."', '".$date_of_birth."', '".addslashes($Address)."',  '".$main_city_id."', '".$main_state_id."','".addslashes($ZipCode)."', '".$country_id."', '".addslashes($Mobile)."','".addslashes($LandlineNumber)."', '".addslashes($PersonalEmail)."', '".$Status."','".addslashes($OtherState)."', '".addslashes($OtherCity)."',  '".addslashes($title)."', '".addslashes($lead_source)."',  '".addslashes($Organization)."', '".addslashes($ReportsTo)."', '".addslashes($AssignTo)."', '".$reference."', '".addslashes($Department)."', '".addslashes($do_not_call)."',  '".addslashes($notify_owner)."' ,  '".$email_opt_out."', '".$PortalUser."','".addslashes($Supp_start_date)."', '".addslashes($Supp_end_date)."', '".addslashes($ipaddress)."', '".addslashes($description)."',  '".$Config['TodayDate']."')";
	...
	/*	function UpdateContact($arryDetails){   
		...
		function UpdateDepartmentHead($ContactID,$depID,$DeptHead){  
			...
			$strSQLQuery = "update department set ContactID='0' where ContactID=".$ContactID; 
			$this->query($strSQLQuery, 0);	


			if($DeptHead==1){
				$strSQLQuery = "update department set ContactID='".$ContactID."' where depID=".$depID; 
				$this->query($strSQLQuery, 0);
			}else{
				$strSQLQuery = "update department set ContactID='0' where depID=".$depID; 
				$this->query($strSQLQuery, 0);
			}
