...
//}
    ...
    function GetBinBySku($Sku) {
        ...
        $strSQLQuery = "select inv_stock.*,w.warehouse_name,w.warehouse_code,w.WID,bin.binlocation_name,binid from inv_bin_stock  inv_stock left outer join w_warehouse w  on inv_stock.Wcode = w.WID left outer join w_binlocation bin  on inv_stock.bin = bin.binid " . $strAddQuery;
...
function GetGenrationBasedOnModel($ModelId)
  ...
  $strSQLQuery = "Select id,Generation from inv_ModelGen where id IN($ModelId)"; 
 
// $arryRow = $this->query($strSQLQuery, 1);
 //$countNo=sizeof($arryRow);
// echo $countNo;exit;
 ///$val = $this->query($strSQLQuery,1);
 ...
 // for($i=0;i<=$countNo;$i++)
 // {
// $aaa +=$arryRow[0]['Generation']+",";
 // }
// return $aaa; exit;
  
   // return $arryRow[0]['Generation'];
