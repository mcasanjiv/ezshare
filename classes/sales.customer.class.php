...
{ 
	...
	function Customer(){
		...
		$this->dbClass();
			...
			$strAddQuery .= ($Config['vAllRecord']!=1)?(" and (c1.AdminType='".$_SESSION['AdminType']."' and c1.AdminID='".$_SESSION['AdminID']."') or c1.SalesID='".$_SESSION['AdminID']."' "):(""); 
		...
		function  GetCustomerContact($CustID,$PrimaryContact,$status=null,$order=' , AddID asc')
		{
		    ...
		    $strSQLQuery = "SELECT ab.* FROM s_address_book ab WHERE ab.CustID='".$CustID."' AND ab.AddType = 'contact' ".$strAddQuery." order by PrimaryContact Desc $order";
		    return $this->query($strSQLQuery, 1);
		}
		...
		function  GetAllAddress($CustID)
		{
			...
			*/
			...
			$sql = "INSERT INTO s_customers SET CustCode = '".mysql_real_escape_string($CustCode)."', 
		...
		function updateCustomerGeneralInfo($arryDetails)
		{
			...
			/***********************/
                ...
                {
                        $SqlCustomer = "SELECT c.*,concat(e.FirstName,' ',e.LastName) as sales_person from s_customers as c
						left join h_employee as e on c.SalesID =e.EmpID  
						WHERE c.Cid = '".$custId."'"; 
		...
		function UpdateSalesPerson($arryDetails){   
			...
			$strSQLQuery = "UPDATE s_customers SET SalesID = '".$arryDetails['SalesPersonID']."' WHERE Cid = '".$CustId."'";
			$this->query($strSQLQuery, 0);
...
*/
				...
				$strSQLQuery = "SELECT c.FullName as CustomerName, c.CustCode, c.Company as CustomerCompany,c.PaymentTerm,c.PaymentMethod,c.ShippingMethod,
				c.Currency,c.Taxable ,ab.FullName as Name,ab.Company,ab.Address, ab.ZipCode, ab.CountryName ,ab.StateName, ab.CityName, 
				...
				LEFT OUTER JOIN h_employee as e on c.SalesID = e.EmpID 
				LEFT OUTER JOIN s_address_book as ab ON (c.Cid = ab.CustID and ab.AddType = 'billing' and ab.Status=1)
				LEFT OUTER JOIN s_address_book as sp ON (c.Cid = sp.CustID and sp.AddType = 'shipping' and sp.Status=1)
