...
class lead extends dbClass
{
		...
		function  ListLead($id=0,$SearchKey,$SortBy,$user,$AscDesc)
		{
			$strAddQuery = '';
			$SearchKey   = strtolower(trim($SearchKey));
			$strAddQuery .= (!empty($id))?(" where l.leadID=".$id):(" where l.Opportunity=0 ");
			$strAddQuery .= (!empty($user))?(" and l.AssignTo=".$user):(" ");
       ...
       function  GetDashboardLead($AdminType,$EmpID)
		{
			...
			$strSQLQuery .= " order by l.leadID desc limit 0,7 ";
		...
		function  GetLead($leadID,$Opportunity)
		{
			...
			//$strSQLQuery = "select l.*,c.cid,c.PID,c.fieldid,c.custom_value from c_lead l  left outer join  c_custom_field_value c on l.leadID=c.PID";
...
/**************************************/
	function  ListTicket($PID,$parent_type,$parentID,$SearchKey,$SortBy,$user,$AscDesc)
		{
			...
			/*if($ParentID>0){
              ...
              $strAddQuery .= (!empty($user))?(" and t.AssignedTo=".$user):(" ");
...
//echo $strSQLQuery;exit;
		...
		function  ListOpportunity($id=0,$SearchKey,$SortBy,$AssignTo,$AscDesc)
		{
			$strAddQuery = '';
			$SearchKey   = strtolower(trim($SearchKey));
			$strAddQuery .= (!empty($id))?(" where o.OpportunityID=".$id):(" where 1 ");
			$strAddQuery .= (!empty($user))?(" and o.AssignTo='".$AssignTo."'"):(" ");
		...
		function UpdateOpportunity($arryDetails){   
			...
			campaignID='".addslashes($campaignID)."',
		...
		#echo $mail->Subject.','.$primary_email.','.$assignEmail.$contents; exit;
			...
			//Send Acknowledgment Email to admin
			$contents = file_get_contents($htmlPrefix."admin_compaign.htm");
            $subject  = "  Added";
			$contents = str_replace("[URL]",$Config['Url'],$contents);
			$contents = str_replace("[SITENAME]",$Config['SiteName'],$contents);
			$contents = str_replace("[FOOTER_MESSAGE]",$Config['MailFooter'],$contents);
			...
			$mail->AddAddress($Config['AdminEmail']);
			$mail->sender($Config['SiteName'], $Config['AdminEmail']);   
			$mail->Subject = $Config['SiteName']." -New  Campaign [".$campaignname."]-[".$CampaignID."] ".$subject;
			$mail->IsHTML(true);
			//echo $Config['AdminEmail'].$contents; exit;
			$mail->Body = $contents;    
			if($Config['Online'] == 1){
