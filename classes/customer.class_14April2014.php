...
{ 
	...
	function Customer(){
		...
		$this->dbClass();
			...
			$strSQLQuery = "select * from e_customers where Email='".$Email."' and Status='Yes'";
			$strSQLQuery .= (!empty($Type))?(" and Type='".$Type."'"):("");
		...
		{
			$sql = "SELECT * FROM e_customers WHERE Email='".$Email."' and Status='Yes'"; 
			...
			if(sizeof($arryRow)>0)
			{
				...
				$sql_u = "UPDATE e_customers SET Password='".md5($new_password)."' WHERE Email='".$Email."'";
				$this->query($sql_u, 0);
                                global $Config;
                                $htmlPrefix = $Config['EmailTemplateFolder'];
                ...
                function  GetSubcribersForEmail($SortBy)
		{
			...
			if($SearchKey=='active' && ($SortBy=='Status' || $SortBy=='') ){
				...
				$strAddQuery .= (!empty($SearchKey))?(" and (Template_Name like '".$SearchKey."%' or Template_Subject ike '".$SearchKey."%' )"):("");
			}
			...
			$strAddQuery .= (!empty($AscDesc))?($AscDesc):(" Desc");
                ...
                {
                     @extract($arryDetails);
                     $SqlCustomer = "INSERT INTO e_newsletter_template SET Template_Subject = '".addslashes($Template_Subject)."', Template_Content = '".addslashes($Template_Content)."', Template_Name = '".addslashes($Template_Name)."', Created_Date = '".$Config['TodayDate']."',Status='".$Status."'";
                     $this->query($SqlCustomer, 1);
                }
                ...
                function updateNewsletterTemplate($arryDetails)
                {
                     @extract($arryDetails);
                      ...
                      $SqlCustomer = "UPDATE e_newsletter_template SET Template_Subject = '".addslashes($Template_Subject)."', Template_Name = '".addslashes($Template_Name)."', Template_Content = '".addslashes($Template_Content)."',Status = '".$Status."' WHERE Templapte_Id =".$TemplateId;
                     return $this->query($SqlCustomer, 0);
                }
