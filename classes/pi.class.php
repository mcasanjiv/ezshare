...
class PieGraph
{
	...
	*/
    function PieGraph($w=200, $h=150, $data)
    {
		...
		//asort($data);
       		$this->data 		= $data ; //array_reverse($data, TRUE);
		$this->pie_width 	= $w;
		$this->pie_height	= $h;
	...
	*/
	function setColors($colors, $bg="FFFFFF", $text="636363", $border="999999")
	{
	...
	*/
    ...
    { 
		...
		*/
		...
		/*******************/
		if(in_array('0',$this->data)){			
			$Config['AddGap']=0.5;
			$TalalVal = array_sum($this->data);

			if($TalalVal>100){
				$Config['AddGap'] = round($TalalVal/100); 				
			}else if($TalalVal>50){
				$Config['AddGap'] = 1; 	
			}
			
			#echo $TalalVal.': '. $Config['AddGap'];exit;
		}
	...
	/*
		Display the graph
	*/
    function display($OptPercentage)
    {
	global $Config;
	$this->init();
	...
	$this->pie = @ImageCreateTrueColor($this->init_img_width, $this->init_img_height);
		...
		#ImageFilledRectangle($leg_img, 0, 0, $leg_width, $leg_height, $colTEXTBG); //pk
		#ImageRectangle($leg_img, 0, 0, $leg_width-1, $leg_height-1, $colTEXTBO); //pk
		...
		foreach($this->data as $k => $data)
		{
			...
			$Val = ($OptPercentage==1)?($percent.'%'):(': '.($data-$Config['AddGap']));
			

			$text = $this->legends[$k]." ".$Val;
			$col = $this->get_color($k, "normal");
		...
		if(!empty($Config['DisplayLabel']))
		{
			ImageString($final, 2, ($this->pie_width+$cellpadding+5), ($this->img_height-ImageFontHeight(2)), $Config['DisplayLabel'], $colTEXT);
		}
