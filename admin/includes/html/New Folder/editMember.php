...
<? if($_GET['opt']=="Seller" && sizeof($arryTemplate)>0) { ?>
	...
	<tr>
	 ...
	 <tr>
		...
		<iframe src="store_templates.php?templateID=<?=$arryMember[0]['templateID']?>&curP=<?=$arryMember[0]['templatePage']?>&templatePage=<?=$arryMember[0]['templatePage']?>" name="TemplateIframe" id="TemplateIframe" frameborder="0" style="border:none; margin:0px 0 0 0;" width="100%" height="380" scrolling="auto" ></iframe>        </td>
      </tr>
	<tr>
    <td  align="center"  >&nbsp;</td>
	</tr>	
...
<? } ?>	 
      ...
      <tr>
		...
		<?
		 if($_GET['opt']=="Seller" && $arryMember[0]['Status']==1) {
      ...
      <tr>
        ...
        <td  height="30" align="left" ><input name="Password" type="text" class="inputbox" id="Password" value="<?php echo stripslashes($arryMember[0]['Password']); ?>" size="30" maxlength="15" />          <span class="blacknormal" >(Password Limit: 5 to 8 characters.) </span></td>
      </tr>
	...
	<? } ?>
  	...
  	<? if($_GET['edit'] > 0 && $_GET['opt']=="Seller" && ($arryMember[0]['WebsiteStoreOption']=='s' || $arryMember[0]['WebsiteStoreOption']=='ws')){?>
      <tr>
...
<tr <? if(empty($_GET['edit'])) echo 'Style="display:none"'; ?>>
	 ...
	 <tr <? if($_GET['opt']!="Seller") echo 'Style="display:none"'; ?>>
...
<input name="LastName" type="text" class="inputbox" id="LastName" value="<?php echo stripslashes($arryMember[0]['LastName']); ?>" size="30" maxlength="50" />            </td>
	...
	?>
            <select name="country_id" class="blacknormal" id="country_id" style="width: 200px;" onchange="Javascript: StateListSend();">
 ...
 <tr>
       ...
       <tr>
        ...
        <td height="30" align="left" ><input name="Phone" type="text" class="inputbox" id="Phone" value="<?php echo $arryMember[0]['Phone']; ?>" size="30" maxlength="20" /> (Country Code, then number) </td>
      </tr>
       <tr>
	...
	<tr>
	    ...
	    <tr>
        <td align="right"   class="blackbold">
		Company Name <? if($_GET['opt']=="Seller") { ?><span class="red">*</span><? } ?> </td>
...
<? if($_GET['opt']=="Seller") { ?>
	 <tr>
        <td align="right"   class="blackbold">VAT Number  </td>
	 ...
	 <tr>
        <td align="right"   class="blackbold">Vat Percentage </td>
	  ...
	  <tr>
        <td align="right"   class="blackbold">Company Tag Line </td>
        <td height="30" align="left" ><input name="TagLine" type="text" class="inputbox" id="TagLine" value="<?php echo stripslashes($arryMember[0]['TagLine']); ?>" size="30" maxlength="50" /></td>
      </tr>
...
<tr>
  <td  align="right"   class="blackbold">Current Logo </td>
	...
	<a href="#" onclick="OpenNewPopUp('../showimage.php?img=upload/company/<?php echo $arryMember[0][Image];?>', 150, 100, 'yes' );"><? echo '<img src="../resizeimage.php?w=70&h=80&img=upload/company/'.$arryMember[0]['Image'].'" border=0 class="imgborder" >';?>			</a>				  </td>
  </tr>  
<?
	}
?>
        ...
        <td height="30" align="left"  class="blacknormal"><input name="Website" type="text" class="inputbox" id="Website" value="<?=(!empty($arryMember[0]['Website']))?($arryMember[0]['Website']):("http://")?>" size="30" maxlength="100" /> (for e.g. http://webo.co.za)</td>
      </tr>
	  ...
	  ?>
       ...
       <table width="100%" border="0" cellspacing="1" cellpadding="4" >
		 ...
		 <input type="checkbox" name="DeliveryStatus<?=$i?>" id="DeliveryStatus<?=$i?>"  value="1" 
                ...
                <td  height="30" align="left" valign="middle" class="blacknormal" ><input name="DeliveryFee<?=$i?>" type="text" class="txtfield" id="DeliveryFee<?=$i?>" value="<?=stripslashes($arryDelivery[$Counter]['Price'])?>"  maxlength="6" size="23" /> <?=$Config['Currency']?>                  </td>
              </tr>
            </table>
		</Div>
		<? } ?>		   
...
function validateBuyer(frm){
			...
			if( ValidateForBlank(frm.UserName, "User Name")
					...
					&& ValidateMandRange(frm.UserName, "User Name", 5, 25)
...
function validateSeller(frm){
		...
		for(var i=1;i<=3;i++){
			...
			if(  ValidateForBlank(frm.UserName, "User Name")
					...
					&& ValidateForPassword(frm.Password, "Password")
					 && isPassword(frm.Password)
					&& ValidateMandRange(frm.Password, "Password",5,8)
					...
					&& ValidateOptNumField(frm.RegistrationNumber, "Registration Number")
					&& ValidateOptPhoneNumber(frm.ContactNumber,"Contact Number")
					&& ValidateOptNumField(frm.VatNumber,"VAT Number")
					&& ValidateOptDecimalField(frm.VatPercentage,"VAT Percentage")
		...
		if(document.getElementById("ShippingDiv").style.display != 'none'){
			if(!ValidateOptDecimalField(frm.Shipping,"Shipping Charge")){
				return false;	
			}
		}*/
