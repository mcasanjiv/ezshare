...
<? 
                        if (is_array($ChimpCampaignList) && $num > 0) {
                            $flag = true;
                            $Line = 0;
                            $LeadNo = 0;
					...
					$status = 'Unsend';
                                            ...
                                            echo '<a href="ViewchimpCampaign.php?Camp_id=' . $values["campaign_id"] .'&S_id='.$values["id"].'" class="'.$status.'"  onclick="Javascript:ShowHideLoader(\'1\',\'P\');">' . $status . '</a>'; 
                                            ?></td>
							<td  align="center" class="head1_inner">
							<a href="ViewchimpCampaign.php?del_id=<?php echo $values['id']; ?>&amp;curP=<?php echo $_GET['curP']; ?>&del_campaign=<?php echo $values['campaign_id']; ?>" onclick="return confirmDialog(this, '<?= $ModuleName ?>')"  ><?= $delete ?></a>   
							</td>
							</tr>
                           <?php } // foreach end //?>
