...
<? } ?>
		...
		$i=0;
			...
			<!--<a href="javascript:void(0);" onclick="jQuery(this).parents(\'li\').find(\'.add-dashboard\').toggleClass(\'active\')">Add To CRM</a> -->
			       <a href="javascript:void(0);" onclick="addtocrm(\''.$i.'\')">Add To CRM</a>|
			       <a href="javascript:void(0);" onclick="addtoexistingcrm(\''.$i.'\')">Existing To CRM</a>
					<!--<input type="checkbox" name="userid[]" value="'.$i.'">--></span></div>';
		...
		echo '<a href="searchContact.php?type=facebook&sid='.$result['id'].'" class="fancybox fancybox.iframe btn-social">Existing Contact</a>';
		}
		
		if(in_array($result['id'],$customer_result)){
		echo '<a href="javascript:void(0)" class="btn-social noactive">Already Existing Customer</a>';  
		}else{	
		echo '<a href="searchCustomer.php?type=facebook&sid='.$result['id'].'" class="fancybox fancybox.iframe btn-social">Existing Customer</a>';
		}
		echo'</div></div>';
