...
echo '</ul>';
	...
	echo '<li>';
	echo '<div class="box-header"><span><img src="'.$result->profile_image_url.'"></span><span class="twitter-profile"><a href="https://twitter.com/'.$result->screen_name.'" target="_blank">Profile</a></span><span class="checkbox-user">
		<a href="javascript:void(0);" onclick="jQuery(this).parents(\'li\').find(\'.add-dashboard\').toggleClass(\'active\')">Add To CRM</a><!--<input type="checkbox" name="userid[]" value="'.$i.'">--></span></div>';
	...
	echo '<div class="add-dashboard">
			<a href="searchContact.php?type=twitter&sid='.$result->id.'&uname='.urlencode($result->screen_name).'&url='.urlencode('https://twitter.com/'.$result->screen_name).'" class="fancybox fancybox.iframe">Add Existing Contact</a>|
			<a href="searchContact.php?type=twitter&sid='.$result->id.'&uname='.urlencode($result->screen_name).'&url='.urlencode('https://twitter.com/'.$result->screen_name).'" class="fancybox fancybox.iframe">Add Existing Customers</a>|
