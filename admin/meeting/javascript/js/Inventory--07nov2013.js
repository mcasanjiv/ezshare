...
function productPickList(currObj,module, row_no) {
	...
	popuptype = 'inventory_prod';
		...
		if(module != ''){
			window.open("select_file.php?curP=1&module=Products&row="+rowId,"productWin","width=640,height=600,resizable=0,scrollbars=0,status=1,top=150,left=200");
}
}
...
// Added For product Reordering starts
	...
	var iMax = tableName.rows.length;
...
document.getElementById("totalProductCount").value = rowCnt;
...
function calcProductTotal(rowId) {	
		...
		var lp=document.getElementById("listPrice"+rowId).value;
		var qt=document.getElementById("qty"+rowId).value;
		var total=eval(qt*lp);
		
		
			var total=eval(document.getElementById("qty"+rowId).value*document.getElementById("listPrice"+rowId).value);
			
			
			document.getElementById("productTotal"+rowId).innerHTML=roundValue(total.toString())

			var totalAfterDiscount = eval(total-document.getElementById("discountTotal"+rowId).innerHTML);
			document.getElementById("totalAfterDiscount"+rowId).innerHTML=roundValue(totalAfterDiscount.toString())
...
//			else
			...
			document.getElementById("netPrice"+rowId).innerHTML=roundValue(netprice.toString())
...
function calcGrandTotal() {
	...
	var discountTotal_final = 0.0,  sh_amount = 0.0, sh_tax = 0.0, adjustment = 0.0;
...
//	alert(netTotal);
	...
	//	txtTaxVal = parseFloat(document.getElementById("txtTax").value);	
	//if(/^-?(0|[1-9]{1}\d{0,})(\.(\d{1}\d{0,}))?$/.test(document.getElementById("txtAdjustment").value))
	//	txtAdjVal = parseFloat(document.getElementById("txtAdjustment").value);
	...
	sh_amount = document.getElementById("shipping_handling_charge").value
...
function validateInventory(module) 
{
	...
	if(max_row_count == 0)
	{
		alert("No line Product Selected");
		return false;
	}
	...
	for (var i=1;i<=max_row_count;i++) 
	{
		...
		if (!emptyCheck("productName"+i,"Please select line product","text")) return false
		...
		if (!emptyCheck("listPrice"+i,"Enter List Price","text")) return false
		if (!numValidate("listPrice"+i,"List Price should be Numeric","any")) return false           
	}
	...
	if(discount_checks[1].checked == true)
	{
		...
		if(!temp)
		{
			alert("Please Enter Valid Final Percent");
			return false;
		}
	}
	if(discount_checks[2].checked == true)
	{
		...
		if(!temp)
		{
			alert("Please Enter Valid Final Amount");
			return false;
		}
	}
	...
	if(!temp)
	{
		alert("Please Enter Valid Shipping Amount");
		return false;
	}
	...
	if(!temp)
	{
		alert("Please Enter Valid Shipping Adjustment");
		return false;
	}
	...
	if(taxtype=="group")
	{
		...
		for(var i=1;i<=tax_count;i++)
		{
			...
			if(!temp)
			{
				alert("Please Enter Valid Tax Percent");
				return false;
			}
		}
	}
	...
	//Taxes for Shippring and Handling  validation - not allow negative values
		...
		for(var i=1;i<=shtax_count;i++)
		{
			...
			if(!temp)
			{
				alert("Please Enter Valid Shipping Tax Percent");
				return false;
			}
		}
...
function FindDuplicate()
{
	...
	product_id[1] = document.getElementById("hdnProductId"+1).value;
	product_name[1] = document.getElementById("productName"+1).value;
	for (var i=1;i<=max_row_count;i++)
	{
		iposition = ""+i;
		for(var j=i+1;j<=max_row_count;j++)
		{
			if(i == 1)
			{
				product_id[j] = document.getElementById("hdnProductId"+j).value;
			}
			if(product_id[i] == product_id[j] && product_id[i] != '')
			{
				...
				if(duplicate_products.search(document.getElementById("productName"+j).value) == -1)
					duplicate_products = duplicate_products+document.getElementById("productName"+j).value+" \n";
			}
		}
	}
	if(duplicate)
	{
		...
		if(!confirm("SELECTED MORE THAN ONCE \n"+duplicate_products+"\n  Want to Continue"))
			return false;
	}
        return true;
}
			...
			onComplete: function(response)
				{
					...
					document.getElementById("taxTotal"+curr_row).innerHTML = document.getElementById('hdnTaxTotal'+curr_row).value;
					calcTotal();
				}
		}
	);
...
var rowCnt=rowCnt+1;
	...
	colone.innerHTML='<img src="images/delete.gif" border="0" onclick="deleteRow(\''+module+'\','+count+',\'images/\')" style="cursor:pointer;"><input id="deleted'+count+'" name="deleted'+count+'" type="hidden" value="0"><br/><br/>&nbsp;<a href="javascript:moveUpDown(\'UP\',\''+module+'\','+count+')" title="Move Upward"></a>';
	/* Product Re-Ordering Feature Code Addition Starts */
	if(iPrevCount != 1)
	{
		oPrevRow.cells[0].innerHTML = '<img src="images/delete.gif" border="0" onclick="deleteRow(\''+module+'\','+iPrevCount+')" style="cursor:pointer;">';
	}
	else
	{
		oPrevRow.cells[0].innerHTML = '<input id="deleted'+iPrevCount+'" name="deleted'+iPrevCount+'" type="hidden" value="0">';
	}
	/* Product Re-Ordering Feature Code Addition ends */
	...
	coltwo.innerHTML= '<table border="0" cellpadding="1" cellspacing="0" width="100%"><tr><td class="small"><input id="productName'+count+'" name="productName'+count+'" class="small" style="width: 70%;" value="" readonly="readonly" type="text">'+
						...
						'&nbsp;<img id="searchIcon'+count+'" title="Products" src="images/products.gif" style="cursor: pointer;" onclick="productPickList(this,\''+module+'\','+count+')" align="absmiddle">'+
						...
						'</td></tr><tr><td class="small" id="setComment'+count+'"><textarea id="comment'+count+'" name="comment'+count+'" class=small style="width:70%;height:40px"></textarea><img src="images/clear_field.gif" onClick="document.getElementById(\'comment'+count+'\').value=\'\'"; style="cursor:pointer;" /></td></tr></tbody></table>';	
	...
	temp='<input id="qty'+count+'" name="qty'+count+'" type="number" class="small " style="width:50px" onfocus="this.className=\'detailedViewTextBoxOn\'" onBlur="settotalnoofrows(); calcTotal(); loadTaxes_Ajax('+count+');';
	if(module == "Invoice")
        {
	...
	colfive.innerHTML='<table width="100%" cellpadding="0" cellspacing="0"><tr><td align="right"><input id="listPrice'+count+'" name="listPrice'+count+'" value="0.00" type="text" class="small " style="width:70px" onBlur="calcTotal();setDiscount(this,'+count+'); calcTotal();"/></td></tr><tr><td align="right" style="padding:5px;" nowrap>(-)&nbsp;<b><a href="javascript:doNothing();" onClick="displayCoords(this,\'discount_div'+count+'\',\'discount\','+count+')" >Discount</a> : </b><div class=\"discountUI\" id=\"discount_div'+count+'"><input type="hidden" id="discount_type'+count+'" name="discount_type'+count+'" value=""><table width="100%" border="0" cellpadding="5" cellspacing="0" class="small"><tr><td id="discount_div_title'+count+'" nowrap align="left" ></td><td align="right"><img src="images/close.gif" border="0" onClick="fnHidePopDiv(\'discount_div'+count+'\')" style="cursor:pointer;"></td></tr><tr><td align="left" class="lineOnTop"><input type="radio" name="discount'+count+'" checked onclick="setDiscount(this,'+count+'); calcTotal();">&nbsp; Zero Discount</td><td class="lineOnTop">&nbsp;</td></tr><tr><td align="left"><input type="radio" name="discount'+count+'" onclick="setDiscount(this,'+count+'); calcTotal();">&nbsp; % Percentage of price </td><td align="right"><input type="text" class="small" size="2" id="discount_percentage'+count+'" name="discount_percentage'+count+'" value="0" style="visibility:hidden" onBlur="setDiscount(this,'+count+'); calcTotal();">&nbsp;%</td></tr><tr><td align="left" nowrap><input type="radio" name="discount'+count+'" onclick="setDiscount(this,'+count+'); calcTotal();">&nbsp; Direct Price Reduction</td><td align="right"><input type="text" id="discount_amount'+count+'" name="discount_amount'+count+'" size="5" value="0" style="visibility:hidden" onBlur="setDiscount(this,'+count+'); calcTotal();"></td></tr></table></div></td></tr><tr> <td align="right" style="padding:5px;" nowrap><b>Total After Discount:</b></td></tr><tr id="individual_tax_row'+count+'" class="TaxShow"><td align="right" style="padding:5px;" nowrap>&nbsp;</td></tr></table> ';
...
function callTaxCalc(curr_row)
{
	...
	if(document.getElementById('tax_table'+curr_row))
	{
...
function calcCurrentTax(tax_name, curr_row, tax_row)
{
	...
	var product_total = document.getElementById("totalAfterDiscount"+curr_row).innerHTML
...
{
        var stock=document.getElementById("qtyInStock"+curr_row).innerHTML;
        var qty=document.getElementById("qty"+curr_row).value;
        if (!isNaN(qty))
        {
                if(eval(qty) > eval(stock))
                document.getElementById("stock_alert"+curr_row).innerHTML='<font color="red" size="1">Stock is not Enough.</font>';
                else
                        document.getElementById("stock_alert"+curr_row).innerHTML='';
        }
        else
     document.getElementById("stock_alert"+curr_row).innerHTML='<font color="red" size="1">Invalid Quantity</font>';
}
...
function updatePrices() {
	...
	if(confirm("CHANGE CURRENCY REVISE UNIT PRICE")) {
		...
		new Ajax.Request(
			...
			{queue: {position: 'end', scope: 'command'},
				method: 'post',
				postBody: 'module=Products&action=ProductsAjax&file=InventoryPriceAjax&productsList='+products_list,
				onComplete: function(response)
					{
...
//function fnAddProductRow(module,image_path) {
//	//alert("Hello");
//	varRow = jQuery("#row1").html();
//	jQuery("#proTab").append("<tr class='newTR'>"+varRow+"</tr>");
//	jQuery(".newTR").find("td:first").html("<img src='images/delete.gif' class='delTR'>");
//}
//
//function decideTaxDiv()
//{
