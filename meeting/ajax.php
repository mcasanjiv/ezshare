...
$objConfig->connect();
  ...
  $agents=$objphone->api('acl_users.php',array());	
	...
	$html['quota'] .='<table width="100%"><tr><td>Call Quota : </td><td>'.$empQuota[0]->q_time.' / '.$empQuota[0]->duration.'</td>
							</tr><tr><td>Total Call : </td><td>'.(!empty($allcalldetail->total)?$allcalldetail->total:0).'</td>
...
if($_POST['action']=='JoinMeeting'){
	$Config['DbName'] = $_SESSION['CmpDatabase'];
	$objConfig->dbName = $Config['DbName'];
	$objConfig->connect();
	$objMeeting = new Meeting();
	if(strlen($_POST['meetingId']) >= 10){
		
		if($data = $objMeeting->getMeetingId($_POST['meetingId'])){
			$bbb = new BigBlueButton();
			
				$joinParams = array(
					'meetingId' => $data['meetingId'], 		   
					'username' => $data['meetingName'],		   
					...
					/*$itsAllGood = true;
					try {$result = $bbb->isMeetingRunningWithXmlResponseArray($data['meetingId']);}
						catch (Exception $e) {
							echo 'Caught exception: ', $e->getMessage(), "\n";
							$itsAllGood = false;
						}
					
					if ($itsAllGood == true && $result['running'][0]=='true') {*/ 
